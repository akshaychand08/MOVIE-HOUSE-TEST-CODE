...
async def help(c, m, cb=False):
        buttons = [[
            ...
            InlineKeyboardButton('Next ➡️', callback_data="mbbss"),
        ]]
        if not cb:
            await m.reply_text(m.text, reply_markup=InlineKeyboardMarkup(buttons), quote=True)


@Client.on_callback_query(filters.regex('^mbbss'))
async def mbbss(query, message): 
    if message.data == "mbbss":
        buttons = [[
            ...
            InlineKeyboardButton('⬅️ Back', callback_data='mbbss+0'),
        ], [
